...
class NMSettingWirelessSecurity (NMSetting):
    ...
    @wep_key0.deleter
    def wep_key0(self):
        if "wep_key0" in self.prop_dict.iterkeys():
            del self.prop_dict["wep-key0"]
    ...
    @wep_key2.deleter
    def wep_key2(self):
        if "wep_key2" in self.prop_dict.iterkeys():
            del self.prop_dict["wep-key2"]
    ...
    @wep_key3.deleter
    def wep_key3(self):
        if "wep_key3" in self.prop_dict.iterkeys():
            del self.prop_dict["wep-key3"]
    ...
    #@wep_key_type.deleter
    #def wep_key_type(self):
        #if "wep-key-type" in self.prop_dict.iterkeys():
            #del self.prop_dict["wep-key-type"]
    ...
    def adapt_wep_commit(self):
        for key in self.prop_dict.keys():
            if key not in ["auth-alg", "key-mgmt", "wep-key-type", "wep-tx-keyidx",
                           "wep-key-flags","wep-key0", "wep-key1", "wep-key2", "wep-key3"]:
                del self.prop_dict[key]

    def adapt_leap_commit(self):
        for key in self.prop_dict.keys():
            if key not in ["auth-alg", "key-mgmt", "leap-username", "leap-password", "leap-password-flags"]:
                del self.prop_dict[key]

    def adapt_dynamic_wep_commit(self):
        '''need adapt 802-1x'''
        for key in self.prop_dict.keys():
            if key not in ["key-mgmt", "pairwise", "proto", "group"]:
                del self.prop_dict[key]
                
    def adapt_wpa_psk_commit(self):
        '''need adapt 802-1x'''                
        for key in self.prop_dict.keys():
            if key not in ["key-mgmt", "psk", "psk-flags"]:
                del self.prop_dict[key]
