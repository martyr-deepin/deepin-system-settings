...
from xdg_support import get_config_dir, get_config_path
from helper import event_manager

from common import VersionError, glib_wait_seconds
...
class SettingsManager(RawConfigParser):
    ...
    def __init__(self, location=None, default_location=None):
        ...
        if self.get_option('settings/version', 0) > self.__version__:
            raise VersionError('Settings version is newer than current.')
...
def add_system_deletes(keys):
    ...
    if deletes:
        ...
        deletes = keys
    global_settings.set_option("settings/system_deletes", deletes)
